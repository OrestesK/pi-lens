...
export function createModuleReportTool(getProjectRoot: () => string) {
	return {
		...
		label: "Module Report",
		description:
			"Source-file outline with symbols, line ranges, callbacks, graph references, risk flags, and recommended reads. " +
			"Falls back to outline-only without a cached review graph. This reports structure, not symbol bodies.",
		promptSnippet:
		...
		parameters: Type.Object({
			...
			view: Type.Optional(
				Type.String({
					...
					description:
						"summary omits heavy detail; default returns JSON; compact returns the full report as line-oriented text.",
				}),
			),
			blastRadius: Type.Optional(
				Type.Boolean({
					description:
						"Include cached-graph transitive dependents as ranked file reads; omitted when cold.",
				}),
			),
...
export function createReadSymbolTool(
	...
	return {
		...
		label: "Read Symbol",
		description:
			"Return one named symbol or callback body with any attached doc comment, and record read coverage. " +
			"Supports `Class.method`; misses suggest nearby names; duplicate names return the first match unless `kind` disambiguates.",
		...
		parameters: Type.Object({
			...
			symbol: Type.String({
				description:
					"Exact symbol name, module_report callback handle, or dotted `Class.method`.",
			}),
			kind: Type.Optional(
				Type.String({
					description:
						"Symbol kind filter for duplicate names; omitted returns the first match.",
				}),
			),
		}),
		async execute(
...
export function createReadEnclosingTool(
	...
	return {
		...
		description:
			"Return the smallest useful symbol or callback enclosing a line and record read coverage. Uses tree-sitter only.",
		...
		parameters: Type.Object({
			...
			onOversize: Type.Optional(
				Type.String({
					...
					description:
						"Oversize handling: error (default) returns metadata; slice returns nearby lines; outline returns nested read handles.",
				}),
			),
